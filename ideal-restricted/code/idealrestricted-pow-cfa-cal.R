...
power.cfa.cal <- function(model1, model2, data, nobs) {
  ...
  mod.cal.1 <- myTryCatch(cfa(model1, sample.cov = sigma.sample, sample.mean = mu.sample, sample.nobs = c(nobs, nobs), group.equal = c("loadings", "intercepts", "residuals")))
  mod.cal.2 <- myTryCatch(cfa(model2, sample.cov = sigma.sample, sample.mean = mu.sample, sample.nobs = c(nobs, nobs), group.equal = c("loadings", "intercepts", "residuals")))
