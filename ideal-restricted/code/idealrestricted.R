...
sapply(packages,library,character.only=T)                     # load all packages
...
source("idealrestricted-cfi.R")
source("idealrestricted-mytrycatch.R")
source("idealrestricted-pow-anova.R")
source("idealrestricted-pow-cfa-cal.R")
source("idealrestricted-pow-cfa-th.R")
source("idealrestricted-pow-manova.R")
source("idealrestricted-pow-sanova.R")
source("idealrestricted-simdata.R")
...
do_sim = function(pos, cond, iters) {
  ...
  if(i == 1){
    write.table(results.cond, file = paste("../results/res_idealres_", results.cond[1,"cond"], "_", results.cond[1,"lambda1"], "_", results.cond[1,"lambda2"], "_", results.cond[1,"lambda3"], "_", results.cond[1,"alpha2"], "_", results.cond[1,"n"], ".dat", sep = ""), row.names=F, col.names=T)
  }
  
  if(i > 1){
    write.table(results.cond, file = paste("../results/res_idealres_", results.cond[1,"cond"], "_", results.cond[1,"lambda1"], "_", results.cond[1,"lambda2"], "_", results.cond[1,"lambda3"], "_", results.cond[1,"alpha2"], "_", results.cond[1,"n"], ".dat", sep = ""), row.names=F, col.names=T, append=T)
  }
  
  # assign NA to string variables first, otherwise mean cannot be calculated
  #results.cond <- sapply(results.cond, as.numeric)
  
  # remove the last four columns (i.e., string variables of warnings and errors) because mean estimate is not accurate
  results.cond <- results.cond[,1:(length(results.cond)-4)]
...
write.table(results.summarized, file = "../results/ideal-restricted_summarized_results.dat", row.names=F, col.names=T)

# write session information to file
writeLines(capture.output(sessionInfo()), "../results/ideal-restricted-sessioninfo.txt")
