### This is simulation code for the condition 'Intercepts'
### OSF:
...
cond_pos <- as.numeric(args[1])

source("intercepts-cfi.R")
source("intercepts-mytrycatch.R")
source("intercepts-pow-anova.R")
source("intercepts-pow-cfa-cal.R")
source("intercepts-pow-cfa-th.R")
source("intercepts-pow-manova.R")
source("intercepts-pow-sanova.R")
source("intercepts-simdata.R")
...
nus <- list(c(0,0,0), c(0,0,0.4), c(0,0,0.6), c(0,0,0.8), c(0,0,1.0))
...
do_sim = function(pos, cond, iters) {
  ...
  nobs <- cond$nobs[pos]
  nus <- unlist(cond$nus[pos])
  
  set.seed(as.numeric(rownames(cond)[pos]))                             
  ...
  for (i in 1:iters) {
    ...
    datalist <- simdata(lambda1,lambda2,lambda3,nus,alpha,nobs,n,q,k)       # list with all data, covar matrices and mean structures
  ...
  # Save results ------------------------------------------------------------
  results.cond <- cbind.data.frame(rep(as.numeric(rownames(cond)[pos]),iters), rep(lambda1,iters), rep(lambda2,iters), rep(lambda3,iters), rep(alpha,iters), rep(nobs,iters), rep(nus[3],iters),
  ...
  colnames(results.cond)[1] <- "cond"; colnames(results.cond)[2] <- "lambda1"; colnames(results.cond)[3] <- "lambda2"; colnames(results.cond)[4] <- "lambda3"; colnames(results.cond)[5] <- "alpha2"; colnames(results.cond)[6] <- "n"; colnames(results.cond)[7] <- "nu";
  results.cond <- as.data.frame(results.cond)
  results.cond.str <- as.data.frame(results.cond.str)
  
  fffname = paste("/home/s421506/intercepts/results/res_inter_", results.cond[1,"cond"], "_", results.cond[1,"lambda1"], "_", results.cond[1,"lambda2"], "_", results.cond[1,"lambda3"], "_", results.cond[1,"alpha2"], "_", results.cond[1,"n"], "_", results.cond[1,"nus"[3]], ".dat", sep = "")
  print(fffname)
  
  write.table(results.cond, file = paste("/home/s421506/intercepts/results/res_inter_", results.cond[1,"cond"], "_", results.cond[1,"lambda1"], "_", results.cond[1,"lambda2"], "_", results.cond[1,"lambda3"], "_", results.cond[1,"alpha2"], "_", results.cond[1,"n"], "_", results.cond[1,"nu"], ".dat", sep = ""), row.names=F, col.names=T)
  write.table(results.cond.str, file = paste("/home/s421506/intercepts/results/res_inter_", results.cond[1,"cond"], "_", results.cond[1,"lambda1"], "_", results.cond[1,"lambda2"], "_", results.cond[1,"lambda3"], "_", results.cond[1,"alpha2"], "_", results.cond[1,"n"], "_", results.cond[1,"nu"], "_str.dat", sep = ""), row.names=F, col.names=T)
  
  
}
  
do_sim(cond_pos, cond, iters)

# write session information to file
writeLines(capture.output(sessionInfo()), "/home/s421506/intercepts/results/intercepts-sessioninfo.txt")
